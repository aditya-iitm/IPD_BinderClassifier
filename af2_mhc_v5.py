...
flags.DEFINE_integer('num_epochs', 100, help='number of epochs')
flags.DEFINE_integer('batch_size', 1, help='total batch size')
flags.DEFINE_integer('print_steps', 100, help='printed averaged results every print_steps')
flags.DEFINE_integer('save_steps', 500, help='save model + optimizer every save_steps')
flags.DEFINE_integer('num_cpus', 8, help='number of cpus for loading')
flags.DEFINE_string('data_path', 'digs', help='digs, perlmutter etc')
flags.DEFINE_integer('crop_size', 200, help='crop sizes to train on')
flags.DEFINE_float('lr_coef', 0.05, help='learning rate coefficient')
flags.DEFINE_float('struc_viol_weight', 1.0, help='structural violation weight')
flags.DEFINE_integer('msa_clusters', 512, help='number of msa cluster sequences')
flags.DEFINE_integer('extra_msa', 1024, help='number of extra msa sequences')
flags.DEFINE_integer('num_evo_blocks', 48, help='number of evoformer blocks')
flags.DEFINE_string('model_weights_path', '/projects/ml/struc2seq/mhc_weights/models/binder_v5_499.pkl', help='path to model weights pkl file')
flags.DEFINE_bool('load_optimizer', True, help='if True, provide optimizer_state_path')
flags.DEFINE_string('optimizer_state_path', '/projects/ml/struc2seq/mhc_weights/models/binder_v5_state_499.pkl', help='path to optimizer state pkl file')
flags.DEFINE_string('global_step_path', '/projects/ml/struc2seq/mhc_weights/models/binder_v5_global_step.npy', help='path to the global step numpy')
flags.DEFINE_string('model_weights_path_save', '/projects/ml/struc2seq/mhc_weights/models/binder_v5', help='save model weights pkl file')
flags.DEFINE_string('optimizer_state_path_save', '/projects/ml/struc2seq/mhc_weights/models/binder_v5_state', help='save optimizer state pkl file')
flags.DEFINE_string('global_step_path_save', '/projects/ml/struc2seq/mhc_weights/models/binder_v5_global_step.npy', help='save the global step numpy')
flags.DEFINE_float('grad_norm_clip', 0.1, help='value to clip gradient norms per example')
...
class BinderClassifier(hk.Module):
    ...
    def __call__(self, input_features, training=False):
        ...
        features = jnp.sum(closest_distances*mask*Ca_mask, -1, keepdims=True)/jnp.sum(mask*Ca_mask, -1, keepdims=True) - 6.0
        ...
        binder_logits = hk.Linear(2)(features)
...
af2_model_params = pickle.load(open(FLAGS.model_weights_path, "rb"))
...
binder_model_params = pickle.load(open('./models/binder_params.pkl', "rb"))
...
def get_feature_dict(item):
	target_crop_size = 180
	target_name = item[0] + '_' + item[1]
	target_name_p = item[0] + '_' + item[2]
	mmcif_object = get_mmcif(item[0], '/home/amotmaen/af2exp/mhcs/cif_mhc')
	target_features = get_all_chain_template_features(mmcif_object, [item[1]])
	peptide_features = get_all_chain_template_features(mmcif_object, [item[2]])
	aligned_sequences, deletion_matrix = parse_a3m(open(f'/home/amotmaen/af2exp/msa/mhc_peptide/{target_name}/t000_.msa0.a3m', 'r').read())
	aligned_sequences = [a[:target_crop_size] for a in aligned_sequences]
	deletion_matrix = [a[:target_crop_size] for a in deletion_matrix]
	aligned_sequences_p, deletion_matrix_p = parse_a3m(open(f'/home/amotmaen/af2exp/msa/mhc_peptide/{target_name_p}/t000_.msa0.a3m', 'r').read())
	...
	processed_feature_dict['pdb_name'] = item
...
def get_feature_dict_binder(k):
        item = ['1k5n', 'A', 'C']
        target_crop_size = 180
        output = pd.read_csv('/home/amotmaen/af2exp/mhcs/ba_dataset.csv')
        target_name = output['allele_msa_file_names'][k]
        labels = np.array([(output['measurement'][k] > 0.6)], np.int32)
        aligned_sequences, deletion_matrix = parse_a3m(open(f'/home/amotmaen/af2exp/msa/mhc_chains/{target_name}/t000_.msa0.a3m', 'r').read())
        query_sequence_p = output['peptide_seq'][k]
        ...
        num_res = pseudo_beta.shape[0]
        processed_feature_dict['backbone_affine_mask'] = np.concatenate([np.ones([1,L1+L2]), np.zeros([1,L-L1-L2])], 1)
        processed_feature_dict['pdb_name'] = output['allele_msa_file_names'][k]
        processed_feature_dict['labels'] = np.eye(2)[labels]
        processed_feature_dict['peptide_mask'] = np.concatenate([np.ones([L1]), np.zeros([L2]), np.zeros([L-L1-L2])], axis=0)
        return processed_feature_dict

class CustomDatasetBinder(torch.utils.data.Dataset):
...
with open('/home/justas/projects/MHC/mhc_train.jsonl', 'r') as json_file:
        json_list = list(json_file)
for json_str in json_list:
    train = json.loads(json_str)

with open('/home/justas/projects/MHC/mhc_test.jsonl', 'r') as json_file:
        json_list = list(json_file)
for json_str in json_list:
    valid = json.loads(json_str)


training_set_binder  = CustomDatasetBinder(list(range(16000)), get_feature_dict_binder)	
training_set  = CustomDataset(list(train.keys()), get_feature_dict, train)			
valid_set  = CustomDataset(list(valid.keys()), get_feature_dict, valid)
params_loader = {
          'shuffle': True,
...
train_loader_binder = torch.utils.data.DataLoader(training_set_binder, **params_loader)
...
if FLAGS.load_optimizer:
	opt_state = pickle.load(open(FLAGS.optimizer_state_path, "rb")) 
	global_step = int(np.load(FLAGS.global_step_path))
else:
...
for e in range(FLAGS.num_epochs):
        ...
        temp_sidechain_fape = []
        for n, combined_batch in enumerate(zip(train_loader, train_loader_binder)):
            for j, batch in enumerate(combined_batch):
                ...
                if (global_step+1) % FLAGS.print_steps == 0:
                    ...
                    print(f'Structure step: {global_step}, loss: {mean_loss}, binder: {binder_loss}, plddt: {plddt}, lddt: {lddt_ca}, fape: {fape}, sc_fape: {sidechain_fape}, chi: {chi_loss}, disto: {distogram}, msa: {masked_msa}', flush=True)
                    temp_train_loss = []
                    temp_lddt_ca = 0.0
                    temp_weights = 0.0
                    temp_distogram = []
                    ...
                    temp_binder_loss_list = []
                    temp_sidechain_fape = []
                if (global_step+1) % FLAGS.save_steps == 0:
                    np.save(FLAGS.global_step_path_save, global_step)
                    save_params = jax.tree_map(lambda x: x[0,], replicated_params)
                    pickle.dump(save_params, open(FLAGS.model_weights_path_save+f'_{global_step}.pkl', "wb"))
                    pickle.dump(opt_state, open(FLAGS.optimizer_state_path_save+f'_{global_step}.pkl', "wb"))
