        ...
        for n, combined_batch in enumerate(zip(train_loader, train_pos_loader)):
            for j, batch in enumerate(combined_batch):
                ...
                if (global_step+1) % 1 == 0:
                    ...
                    print(f'Structure step: {global_step}, loss: {mean_loss}, binder: {binder_loss}, plddt: {plddt}, lddt: {lddt_ca}, fape: {fape}, sc_fape: {sidechain_fape}, chi: {chi_loss}, disto: {distogram}, msa: {masked_msa},pdb: {batch["pdb_name"]}, label: {batch["labels"]}, prob1: {probs[1]}', flush=True)
                    temp_train_loss = []
                    temp_lddt_ca = 0.0
                    temp_weights = 0.0
                    temp_distogram = []
                ...
                if (global_step+1) % 500 == 0:
                    np.save('/net/scratch/aditya20/af2exp/model/af2_ssm_v3_global_step.npy', global_step)
                    save_params = jax.tree_map(lambda x: x[0,], replicated_params)
                    pickle.dump(save_params, open('/net/scratch/aditya20/af2exp/model/binder_v3'+f'_{global_step}.pkl', "wb"))
                    pickle.dump(opt_state, open('/net/scratch/aditya20/af2exp/model/binder_v3'+f'_{global_step}_state.pkl', "wb"))
