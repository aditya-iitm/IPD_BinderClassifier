...
optimizer_state_path = './model/binder_v1_state_5799.pkl'
global_step_path     = './model/af2_ssm_v1_global_step.npy'
model_weights_path   = './model/binder_v1_5799.pkl'
...
model_config.data.common.max_extra_msa = 128
model_config.data.eval.max_msa_clusters = 128
model_config.data.eval.crop_size = 300
...
#UNCOMMENT ABOVE LINES FOR INITIAL RUN
...
def get_loss_fn(binder_model_params, af2_model_params, key, processed_feature_dict, structure_flag):
    classifier_dict = {}
...
def train_step(binder_model_params, af2_model_params, key, batch, structure_flag):
    ...
    (loss, (predicted_dict, binder_loss, prob, features)), grads = jax.value_and_grad(get_loss_fn, has_aux=True)(binder_model_params, af2_model_params, key, batch, structure_flag)
...
def get_feature_dict_binder(item):
        ...
        target_name = item[2]
        aligned_sequences, deletion_matrix = parse_a3m(open(f'/home/aditya20/experimentsWaf2/ssm_parent_a3m/{target_name}/t000_.msa0.a3m', 'r').read())
        ...
        #print(aligned_sequences,aligned_sequences_p)
...
gradient_transform = optax.chain(
    ...
    optax.scale(-1.0*0.05) #lr-coeff
)

n_devices = jax.local_device_count()
replicated_binder_model_params = jax.tree_map(lambda x: jnp.array([x] * n_devices), binder_model_params)
replicated_af2_model_params = jax.tree_map(lambda x: jnp.array([x] * n_devices), af2_model_params)
...
for e in range(10):
        ...
        for n, combined_batch in enumerate(zip(train_loader, train_pos_loader)):
            for j, batch in enumerate(combined_batch):
                ...
                loss, grads, predicted_dict, binder_loss, prob, features = jax.pmap(train_step, in_axes=(0,0,None,0,None), axis_name='model_ax')(replicated_binder_model_params, replicated_af2_model_params, subkey, batch, structure_flag)
                print(jax.pmap(train_step, in_axes=(0,0,None,0,None), axis_name='model_ax')(replicated_binder_model_params, replicated_af2_model_params, subkey, batch, structure_flag))
                break
                ...
                if (global_step+1) % 1 == 0:
                    ...
                    print(f'Structure step: {global_step}, loss: {mean_loss}, binder: {binder_loss}, plddt: {plddt}, lddt: {lddt_ca}, fape: {fape}, sc_fape: {sidechain_fape}, chi: {chi_loss}, disto: {distogram}, msa: {masked_msa}, pdb: {pdb_name}, label: {batch["labels"]}, prob1: {prob[1]}', flush=True)
                    temp_train_loss = []
                    temp_lddt_ca = 0.0
                    temp_weights = 0.0
                    temp_distogram = []
                ...
                if (global_step+1) % 100 == 0:
                    np.save('./model/af2_ssm_v2_global_step.npy', global_step)
                    save_params = jax.tree_map(lambda x: x[0,], replicated_params)
                    pickle.dump(save_params, open('./model/binder_v2'+f'_{global_step}.pkl', "wb"))
                    pickle.dump(opt_state, open('./model/binder_v2'+f'_{global_step}_state.pkl', "wb"))
